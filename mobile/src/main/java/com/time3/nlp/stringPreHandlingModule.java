...
public class stringPreHandlingModule {
	...
	public static String numberTranslator(String target){
		Pattern p = Pattern.compile("[一幺二两三四五六七八九123456789]万[一幺二两三四五六七八九123456789](?!(千|百|十))");
		Matcher m = p.matcher(target); 
		StringBuffer sb = new StringBuffer(); 
		boolean result = m.find(); 
		while(result) { 
		...
		p = Pattern.compile("[一幺二两三四五六七八九123456789]千[一幺二两三四五六七八九123456789](?!(百|十))");
		m = p.matcher(target); 
		sb = new StringBuffer(); 
		result = m.find(); 
		while(result) { 
		...
		p = Pattern.compile("[一幺二两三四五六七八九123456789]百[一幺二两三四五六七八九123456789](?!十)");
		m = p.matcher(target); 
		sb = new StringBuffer(); 
		result = m.find(); 
		while(result) { 
		...
		p = Pattern.compile("[零一幺二两三四五六七八九]");
		m = p.matcher(target); 
		sb = new StringBuffer(); 
		result = m.find(); 
		while(result) { 
	...
	private static int wordToNumber(String s){
		if(s.equals("零")||s.equals("0"))
			return 0;
		else if(s.equals("一")||s.equals("幺")||s.equals("1"))
			return 1;
